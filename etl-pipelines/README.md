...
| [03-configuration.md](03-configuration.md) | YAML vs Python config, environments, country config Protocol, TypedDict |
| [04-data-types-and-validation.md](04-data-types-and-validation.md) | Typed dataclasses, enums, data integrity checks |
| [05-extract.md](05-extract.md) | Extracting from APIs, Storage abstraction, async downloads, Pydantic validation |
| [06-transform.md](06-transform.md) | Business logic separation, idempotency, pandas vs pure Python |
| [07-load.md](07-load.md) | Output modes, API loading, file output, integrity checks before load |
...
| **Enum (StrEnum)** | An enum defines a fixed set of allowed values, such as environments or output modes. This prevents typo-based bugs and makes config validation and autocomplete better. |
...
| **Environment** | An environment is a run profile such as debug, test, or prod that controls data sources and output behavior. It allows the same codebase to run safely in different settings by changing configuration, not code. |
| **Route** | A route is one unit of work processed by a pipeline run — typically a country or region with its own data sources, target level, and output. Errors are accumulated per route so one failure does not block the others. |
